...
## Iteration 20（文件生成面板，已完成）

- 轮次：Iteration 20（文件生成面板）
- 状态：已完成
- 负责角色：前端开发者

## 背景

用户反馈：「实际类似 Claude 要生成对应文件与处理项目」，希望在生成过程中显示文件列表和状态。

## 改动内容

**src/pages/HomePage.tsx** 增强：

1. **文件面板组件**：
   - 新增 `FileTreeItem` 组件，显示单个文件
   - 文件状态：pending（待创建）/ generating（生成中）/ completed（已完成）
   - 显示文件名、状态图标、文件大小

2. **生成状态面板重构**：
   - 阶段进度：图标 + 标题 + 进度条
   - 文件面板：项目文件树（当前 index.html）
   - 流式输出：显示最近 500 字符的生成内容
...
- 构建通过：✅（731.23 KB JS / 44.82 KB CSS）
- 文件面板显示：✅
- 状态变化：✅
- 文件大小计算：✅
...
│ 📁 项目文件                             │
│  ○ index.html     待创建                │
│  ⟳ index.html     生成中 2.3KB          │
│  ✓ index.html     已完成 8.5KB          │
├────────────────────────────────────────┤
│ ● 正在生成...                          │
│ <!DOCTYPE html>...                     │
└────────────────────────────────────────┘
...
1. **文件面板是标配**：类似 Claude Artifacts / Cursor 等工具都会显示正在生成的文件
2. **状态可视化**：pending → generating → completed 三态变化让用户清晰感知进度
3. **文件大小实时更新**：让用户看到生成内容正在增长
4. **流式输出窗口**：显示最近内容而非全部，避免界面过长

## 后续扩展点

1. 多文件支持：让 AI 生成 CSS/JS 分离的多文件项目
2. 文件树展开：支持文件夹结构
3. 文件编辑：点击文件可在代码 Tab 中查看/编辑

## Iteration 19（项目空间页面，已完成）

- 轮次：Iteration 19（项目空间页面）
- 状态：已完成
- 负责角色：前端开发者

## 背景

用户反馈：「生成的内容需要有个项目空间，存储起来」，参考秒哒的项目管理交互。

## 改动内容

1. **新增 ProjectsPage.tsx**：
   - 项目卡片网格布局（缩略图 + 名称 + 状态 + 时间）
   - 卡片菜单：打开 / 删除操作
   - 空状态引导：创建第一个项目
...
- 构建通过：✅（727.62 KB JS / 44.88 KB CSS）
- 项目空间页面路由：✅
- 项目卡片显示：✅
- 打开/删除操作：✅
...
1. **项目空间是标配**：类似秒哒/Cursor 等 AI 生成工具都有项目空间，用户需要管理历史生成
2. **卡片式展示**：缩略图 + 名称 + 状态 + 时间是标准卡片结构
3. **空状态引导**：无项目时应引导用户创建，而非空白页面
4. **时间格式化**：相对时间（刚刚、X 分钟前）比绝对时间更友好

## Iteration 18（全屏智能体界面，已完成）

- 轮次：Iteration 18（全屏智能体界面重构）
- 状态：已完成
- 负责角色：前端开发者

## 背景

用户反馈：「这个是一个智能体，类似秒哒那种」，希望 HomePage 改为全屏布局，左侧 AI 对话 + 右侧实时预览。
...
1. **布局重构**：
   - 从居中输入框改为全屏双栏布局
   - 左侧：AI 对话区（欢迎消息 + 模板芯片 + 流式输出 + 输入框）
   - 右侧：预览/代码双 Tab（iframe 沙箱 + 代码查看）

2. **流式输出增强**：
   - 阶段指示器图标（search/code-2/eye/check-circle）
   - 三阶段进度条（analyzing → generating → reviewing）
   - 流式文本实时显示 + 闪烁光标

3. **预览区功能**：
   - 预览 Tab：iframe 沙箱实时显示生成的应用
   - 代码 Tab：HTML 语法高亮 + 文件大小显示
   - 下载按钮：一键下载生成的 HTML 文件
...
- 构建通过：✅（718.91 KB JS / 43.64 KB CSS）
- 双栏布局显示：✅
- 流式输出效果：✅
- 预览/代码 Tab 切换：✅
- 移动端单栏响应式：✅
...
1. **智能体产品形态**：类似秒哒/Cursor 的全屏双栏布局是主流形态，用户已形成认知习惯
2. **流式输出视觉**：进度条 + 阶段图标 + 闪烁光标三者结合，让用户清晰感知 AI 工作状态
3. **预览即所得**：首页就能看到生成效果，无需跳转，体验更流畅
4. **代码可见性**：代码 Tab 满足开发者查看实现细节的需求

## Iteration 17（SSE 流式输出修复，已完成）

- 轮次：Iteration 17（SSE 流式输出修复）
- 状态：已完成
- 负责角色：AI 工程师

## 背景

用户反馈：「目前一直显示：'正在生成代码...'。这个地方都交互需要流式效果，在后台有持续输出与响应，直到完整」

## 问题诊断

1. **Vite 代理对 SSE 支持不完善**：开发环境下前端通过 Vite 代理请求后端，但代理层对 SSE 流式传输有兼容问题
2. **SSE 解析逻辑健壮性不足**：原有解析依赖 `\n\n` 分隔符定位事件块，对不完整数据块处理有缺陷

## 修复内容

1. **server/index.ts**：CORS 白名单新增 5175、5176 端口
2. **src/services/ai/liveEngine.ts**：
   - 开发环境直接请求 `http://localhost:3000`，绕过 Vite 代理
   - 重写 SSE 解析：逐行解析（`buffer.split('\n')`），识别 `event:` / `data:` 行，空行触发事件处理
   - 新增 `processSSEEvent()` 独立函数处理单个 SSE 事件
   - 增加调试日志：`[liveEngine] 收到 chunk X 长度: Y`、`[liveEngine] 解析事件: ...`

## 验证结果

- curl 直测后端 SSE：正常输出 `event: stage` / `event: delta` 序列 ✅
- CORS 白名单：5176 端口已包含 ✅
- 前端代码：apiBase 变量正确指向后端 ✅
- SSE 解析逻辑：逐行解析实现完成 ✅

## 经验总结

1. **Vite 代理对 SSE 支持有限**：开发环境下 SSE 请求应直接走后端 URL，不经过 Vite 代理
2. **SSE 解析要逐行处理**：不要依赖 `\n\n` 一次性定位事件块，流式数据可能分块到达，逐行解析更健壮
3. **调试日志是关键**：在流式处理的关键节点添加 console.log，便于定位问题
