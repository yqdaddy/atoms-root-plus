...
- **只携带最新一份 HTML**：上下文中永远只有当前版本的 HTML，丢弃历史版本；对话历史压缩为「原始需求 + 最近 5 条修改指令」的纯文本，不携带历次 AI 回复全文。
- **迭代请求构造**：分析师带 `{{RECENT_CONTEXT}}`（此前 summary）产出修改计划；工程师使用 2.2 的修复轮 User 模板：`{{CURRENT_HTML}}` + `{{REPAIR_INSTRUCTIONS}}`（本次为修改计划转化的指令）+ `{{USER_PROMPT}}`，System 附加约束"未涉及部分保持原样"以抑制无谓改动。
- **预检算力**：发起前用第 7 节估算器算 input 预算，当前 HTML 超过 40k 字符时提示用户"应用较大，建议新开应用"。
