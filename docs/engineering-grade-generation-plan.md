...
1. 全局命名空间：采用本文默认 `window.__components`。
2. README 定位：初始说明，不随 modify 重生成（P2 增强层一并考虑）。
3. W_FILE_SIZE 150 行阈值：采纳本文建议值（P1 落地时生效）。
4. P2 文档增强层触发方式：留待 P2 立项时决策。
5. 编译产物交付（新增决策，方案 B）：ZIP 导出时由平台将浏览器端编译产物物化为 /dist/index.html，与源码工程一并交付；P1 实施。
6. 启动范围：P0 + P1 连做。布局滚动纪律规则（单屏应用适配视口 / 内容流应用自然滚动 / 全应用禁横向滚动）并入 P0 提示词改造，不单独立项。
