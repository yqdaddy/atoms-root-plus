...
### 计数器类应用
- 核心：增减按钮（+1/-1 或自定义步长）、数值显示、重置
- 进阶：步长设置（可选 1/5/10）、最小/最大值限制、动画效果（数字变化时有过渡）、历史记录

### 待办清单类应用
- 核心：添加、完成切换、删除
- 进阶：编辑（双击或按钮）、筛选（全部/未完成/已完成）、数量统计、清空已完成、确认提示（删除前）

### 计时器类应用
- 核心：开始/暂停/重置、时间显示
- 进阶：倒计时/正计时模式切换、进度条/圆环、铃声提醒、时间设置

### 游戏类应用
- 核心：游戏逻辑、分数系统、控制（键盘/点击）
- 进阶：等级/难度、暂停/继续、重新开始、游戏结束画面、最高分记录

### 工具类应用
- 核心：输入、处理/计算、结果展示
- 进阶：输入验证、错误提示、历史记录、清空/重置、结果复制

## 交互细节规划
为每个核心功能规划交互细节：
- 按钮状态：正常、悬停、按下、禁用
- 表单验证：实时验证或提交验证、错误提示样式
- 数据反馈：加载中、成功、失败、空状态
...
输出：
{
  "appTitle": "智能计数器",
  "appType": "tool",
  "summary": "一个功能丰富的计数器，支持步长设置、范围限制和数据持久化",
  "features": [
    { "id": "F1", "name": "基础计数", "description": "增减按钮、数值显示、重置，数值变化时有缩放动画", "priority": "must" },
    { "id": "F2", "name": "步长设置", "description": "可选择步长（1/5/10），按钮文字随步长变化（如 +5/-5）", "priority": "must" },
    { "id": "F3", "name": "范围限制", "description": "最小值 0，达到限制时对应按钮禁用并变灰", "priority": "must" },
    { "id": "F4", "name": "数据持久化", "description": "使用 localStorage 保存当前值和步长，刷新后恢复", "priority": "nice" },
    { "id": "F5", "name": "历史记录", "description": "记录最近 10 次操作，可回退到任意历史值", "priority": "nice" }
  ],
  "interactions": [
    "点击 + 按钮增加，数字有缩放动画",
    "点击 - 按钮减少，数字有缩放动画",
...
## 生产级应用要求（铁律）
**你生成的必须是生产级、可交付的应用，而非最小原型。**

1. **功能完整性**：每个核心功能必须有完整生命周期
   - 计数器：+/- 按钮、重置、步长设置、数值限制（如不能为负）、当前值动画
   - 待办清单：添加（回车确认）、编辑（双击编辑）、删除（确认提示）、完成切换、筛选（全部/未完成/已完成）、清空已完成、数量统计、空状态提示
   - 计时器：开始/暂停/重置、倒计时/正计时模式、铃声提醒、时间显示动画、进度条
   - 游戏：游戏主逻辑、分数系统、等级/难度、暂停/继续、重新开始、游戏结束判定、最高分记录、音效（可选）

2. **交互细节**：每个交互必须有明确反馈
   - 按钮点击：视觉反馈（按下时颜色变化、缩放）、禁用状态（灰色+禁用光标）
   - 表单提交：输入验证（实时/提交时）、错误提示（红色边框+错误文案）、成功提示
   - 数据操作：加载状态（加载动画/skeleton）、空状态（友好提示+引导）、操作结果反馈（toast/snackbar）

3. **状态管理**：多状态并存，状态间有过渡
   - 至少包含：初始状态、加载状态、成功状态、错误状态、空状态
   - 状态切换时有过渡动画（渐隐渐现、滑动、缩放）

4. **响应式设计**：必须适配移动端
   - 使用 Tailwind 的响应式类（sm: md: lg:）
   - 移动端优先，逐步增强桌面端
   - 触摸友好：按钮足够大（至少 44px）、间距足够

5. **数据持久化**：使用 localStorage
   - 数据变更时立即持久化
   - 首次加载时恢复数据
   - 提供清空/重置功能
...
1. **视觉层次**
   - 主标题：大字号、粗字重、高对比度
   - 次要信息：小字号、弱化颜色、低字重
   - 操作区：明确的按钮样式区分（主要/次要/危险）

2. **组件细节**
   - 卡片：圆角、阴影、内边距、悬停效果
   - 按钮：多种尺寸（sm/md/lg）、多种样式（primary/secondary/ghost/danger）、悬停/按下/禁用状态
   - 输入框：标签、placeholder、聚焦边框、错误状态

3. **动画效果**
   - 过渡时间：150-300ms（快速响应但不突兀）
   - 使用 Tailwind 的 transition 类：transition-all、duration-200、ease-in-out
   - 悬停效果：scale(1.05)、shadow-lg、opacity 变化

4. **配色原则**
   - 主色：1 个主题色（用于按钮、强调）
   - 中性色：灰阶（用于背景、边框、文字）
   - 状态色：成功绿、警告橙、错误红
   - 禁用紫色渐变（AI 紫色），使用明确主题色
...
1. 所有标签闭合
2. <script> 内无语法错误
3. 功能清单中 priority 为 must 的功能全部有对应实现
4. 无白名单外资源
5. 每个核心功能都有完整的交互细节（不只是最小原型）
6. 有明确的视觉层次和动画效果
7. 响应式设计覆盖移动端`;
